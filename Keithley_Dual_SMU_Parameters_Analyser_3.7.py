...
def get_compliance_value():
    ...
    try:
        ...
        elif compliance_str.endswith("A"):
            if compliance_str.endswith("nA") or compliance_str.endswith("µA") or compliance_str.endswith("mA"):
...
def run_srdp_characterization():
    ...
    try:
        ...
        base_params = {
            ...
            "read_width_ms": validate_float(read_width_entry.get(), "Read Pulse Width"),
            "compliance_A": get_compliance_value(),
            "nplc": validate_float(nplc_entry.get(), "NPLC"),
            "settle_ms": 5,
            "measure_avg": 1,       
        ...
        if simulate_var.get():
            ...
            try:
                ...
                stim_current = abs(base_params['stim_level'])
                if "2612B" in model and stim_current > 1.5:
                    instrument.close()
                    messagebox.showerror(
                        "Current Limit Exceeded - SRDP",
                        f"ERROR: 2612B maximum current is 1.5A\n\n"
                        f"SRDP Stim Level: {stim_current}A\n\n"
                        f"Please reduce to ≤1.5A or use 2636A"
                    )
                    return
            except Exception as e:
    ...
    try:
        ...
        if simulate_var.get():
            ...
            try:
                ...
                # Check current capability (BOTH pre and post spikes)
                pre_current = abs(base_params['stim_level'])
                post_current = abs(base_params.get('post_spike_level', pre_current))
                max_current = max(pre_current, post_current)
                
                if "2612B" in model and max_current > 1.5:
                    instrument.close()
                    messagebox.showerror(
                        "Current Limit Exceeded - STDP",
                        f"ERROR: 2612B maximum current is 1.5A\n\n"
                        f"Pre-spike: {pre_current}A\n"
                        f"Post-spike: {post_current}A\n\n"
                        f"Please reduce both to ≤1.5A or use 2636A"
                    )
                    return
            except Exception as e:
                print(f"⚠️  Warning: Could not detect instrument model: {e}")
    ...
    try:
        ...
        if simulate_var.get():
            ...
            try:
                ...
                # Check current capability for both pot and dep
                pot_current = abs(pot_params['stim_level'])
                dep_current = abs(dep_params['stim_level'])
                max_current = max(pot_current, dep_current)
                
                if "2612B" in model and max_current > 1.5:
                    instrument.close()
                    messagebox.showerror(
                        "Current Limit Exceeded - Cycles",
                        f"ERROR: 2612B maximum current is 1.5A\n\n"
                        f"Potentiation: {pot_current}A\n"
                        f"Depression: {dep_current}A\n\n"
                        f"Please reduce both to ≤1.5A or use 2636A"
                    )
                    return
            except Exception as e:
        ...
        if simulate_var.get():
            ...
            try:
                ...
                if "2612B" in model and max_current > 1.5:
                    instrument.close()
                    messagebox.showerror(
                        "Current Limit Exceeded - Multi-Device",
                        f"ERROR: 2612B maximum current is 1.5A\n\n"
                        f"Potentiation: {pot_current}A\n"
                        f"Depression: {dep_current}A\n\n"
                        f"Cannot proceed with {n_devices} devices.\n"
                        f"Reduce current to ≤1.5A or use 2636A"
                    )
                    return
            except Exception as e:
                print(f"⚠️  Warning: Could not detect instrument model: {e}")
...
ctk.CTkLabel(synapse_frame, text="Read Settle Delay (ms):").pack(anchor="w", padx=10)
read_settle_delay_entry = ctk.CTkEntry(synapse_frame)
read_settle_delay_entry.pack(fill="x", padx=10, pady=5)
read_settle_delay_entry.insert(0, "5")

ctk.CTkLabel(synapse_frame, text="Read Pulse Width (ms):").pack(anchor="w", padx=10)
read_width_entry = ctk.CTkEntry(synapse_frame)
