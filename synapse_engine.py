...
def validate_instrument_model(instrument):
    """
    Check connected instrument model.
...
def check_10a_range_support(instrument):
    """
    Returns True if instrument supports 10A range.
    Only 2636A/2636B support 10A range.
    2612B maximum is 1.5A.
    """
    model = validate_instrument_model(instrument)
    if "2636A" in model or "2636B" in model:
        return True
    return False

def get_max_current(instrument):
    """
    Returns maximum current capability of connected instrument.
    """
    model = validate_instrument_model(instrument)
    if "2636A" in model or "2636B" in model:
        return 10.0  # 10A in pulse mode
    else:
        return 1.5  # 1.5A for 2612B and similar
...
def pulse_read_sequence(instrument, stim_ch, read_ch, params):
    ...
    if params['stim_drive_type'] == 'I':  # Current sourcing mode
        ...
        if requested_current > max_current:
            ...
            raise ValueError(
                ...
                f"Please use 2636A for currents above 1.5A, or reduce current level."
            )
    
    # --- CONFIGURATION PHASE ---
    
    # 0. FIRST: Configure sense mode (MUST be before channel configuration)
    if 'wire_mode' in params:
        if params['wire_mode'] == '4-Wire':
            instrument.write(f"{read_ch}.sense = {read_ch}.SENSE_REMOTE")
            instrument.write(f"{stim_ch}.sense = {stim_ch}.SENSE_REMOTE")
        else:
            instrument.write(f"{read_ch}.sense = {read_ch}.SENSE_LOCAL")
            instrument.write(f"{stim_ch}.sense = {stim_ch}.SENSE_LOCAL")
    ...
    nplc = params.get('nplc', 1.0)  # Use GUI NPLC or default to 1.0
    ...
    if params['stim_drive_type'] == 'V':
        ...
        instrument.write(f"{stim_ch}.source.autorangev = {stim_ch}.AUTORANGE_ON")
        instrument.write(f"{stim_ch}.measure.autorangei = {stim_ch}.AUTORANGE_ON")
        instrument.write(f"{stim_ch}.source.limiti = {params['compliance_A']}")
    else:
        stim_func = f"{stim_ch}.OUTPUT_DCAMPS"
        instrument.write(f"{stim_ch}.source.func = {stim_func}")
        instrument.write(f"{stim_ch}.source.autorangei = {stim_ch}.AUTORANGE_ON")
        instrument.write(f"{stim_ch}.measure.autorangev = {stim_ch}.AUTORANGE_ON")
        instrument.write(f"{stim_ch}.source.limiti = {params['compliance_A']}")
        # When sourcing current, set voltage compliance
        max_voltage = 10.0  # Safe limit
        instrument.write(f"{stim_ch}.source.limitv = {max_voltage}")
    ...
    # Calculate averaging parameters ONCE
    read_width_ms = params.get('read_width_ms', 50)
    nplc = params.get('nplc', 1.0)
    time_per_measurement_ms = nplc * (1000.0 / 60.0)
    n_samples = max(1, int(read_width_ms / time_per_measurement_ms))
    
    # === CONFIGURE HARDWARE FILTER ONCE (outside loop) ===
    if n_samples > 1:
        # Practical limit: 2-50 samples for good balance of speed vs. noise rejection
        # Instrument hard limit is ~100, but beyond 30-50 samples gives diminishing returns
        n_samples_clamped = max(2, min(n_samples, 50))
    ...
    try:
        for i in range(params['n_pulses']):
            ...
            # Wait for read settle delay (device stabilization time)
            read_settle_delay_ms = params.get('read_settle_delay_ms', 5)
            time.sleep(read_settle_delay_ms / 1000.0)
            
            # === Single query with hardware averaging (filter already configured) ===
            I_meas = float(instrument.query(f"print({read_ch}.measure.i())").strip())
...
def simulate_pulse_read(params):
    ...
    if params['stim_drive_type'] == 'I':
        ...
        if requested_current > 1.5:
            ...
            print(f"    This exceeds 2612B capability (max 1.5A).")
            print(f"    Use 2636A for currents above 1.5A.")
...
def measure_srdp(instrument, stim_ch, read_ch, base_params, freq_list_hz):
    ...
    # Check instrument current capability
    max_current = get_max_current(instrument)
...
def measure_stdp(instrument, stim_ch, read_ch, base_params, delta_t_list_ms):
    ...
    # Configure sense mode (4-wire/2-wire)
    if 'wire_mode' in base_params:
        if base_params['wire_mode'] == '4-Wire':
            instrument.write(f"{stim_ch}.sense = {stim_ch}.SENSE_REMOTE")
            instrument.write(f"{read_ch}.sense = {read_ch}.SENSE_REMOTE")
        else:
            instrument.write(f"{stim_ch}.sense = {stim_ch}.SENSE_LOCAL")
...
def simulate_srdp(base_params, freq_list_hz):
    ...
    for freq_hz in freq_list_hz:
        # Calculate period from frequency (REAL LOGIC)
        period_ms = 1000.0 / freq_hz
        
        # Check minimum period constraint (REAL VALIDATION)
        if period_ms < min_period:
            print(f"  ⚠️  Skipping {freq_hz:.1f} Hz (period {period_ms:.1f} ms < min {min_period:.1f} ms)")
            continue
