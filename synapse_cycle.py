...
def run_potentiation_depression_cycle(instrument, stim_ch, read_ch, pot_params, dep_params, n_cycles, 
    ...
    # Check potentiation current
    pot_current = abs(pot_params.get('stim_level', 0))
    if pot_current > max_current:
        model = synapse_engine.validate_instrument_model(instrument)
        raise ValueError(
            f"ERROR: Potentiation current {pot_current}A exceeds instrument capability.\n"
            f"Connected: {model} (Max: {max_current}A)"
        )
    
    # Check depression current
    dep_current = abs(dep_params.get('stim_level', 0))
    if dep_current > max_current:
        model = synapse_engine.validate_instrument_model(instrument)
        raise ValueError(
            f"ERROR: Depression current {dep_current}A exceeds instrument capability.\n"
            f"Connected: {model} (Max: {max_current}A)"
        )
